...
//微信参数配置
    ...
    public static String httpsRequest2(String requestUrl, String requestMethod, String outputStr) throws Exception {
        ...
        FileInputStream instream = new FileInputStream(new File("/www/server/apache-tomcat-8.5.32/cert/apiclient_cert.p12"));
        try {
            keyStore.load(instream, "1510739681".toCharArray());
        } finally {
            instream.close();
        }

        // Trust own CA and all self-signed certs
        SSLContext sslcontext = SSLContexts.custom().loadKeyMaterial(keyStore, "1510739681".toCharArray()).build();
        // Allow TLSv1 protocol only
        SSLConnectionSocketFactory sslsf = null;//new SSLConnectionSocketFactory(sslcontext, new String[] { "TLSv1" }, null, SSLConnectionSocketFactory.BROWSER_COMPATIBLE_HOSTNAME_VERIFIER);
